...
Friendship exposes synchronous relationship command and bounded read contracts. `gateway` is the primary caller for end-user request, accept, reject, remove, and block flows. These calls stay synchronous for immediate correctness and block enforcement, not because they share chat/realtime latency targets.

## Shared Contract Rules

- Authenticated actor identity comes from `gateway`; callers must not be allowed to mutate another actor's relationship state by supplying arbitrary user IDs.
- Friendship enforces target-user existence at its own boundary by synchronously validating write-path target IDs with `identity`; `gateway` may pre-validate, but friendship must not rely on gateway-only checks.
...
- `actor_user_id` (`uuid`) - authenticated requester.
...
- `actor_user_id` (`uuid`) - authenticated addressee accepting the request.
- `friend_request_id` (`uuid`)
...
- `actor_user_id` (`uuid`) - authenticated addressee rejecting the request.
- `friend_request_id` (`uuid`)
...
- `actor_user_id` (`uuid`) - authenticated blocker.
...
- Create or preserve the directional `user_block` row for `(actor_user_id, target_user_id)`.
...
- Returns only accepted friendship edges where `user_id = actor_user_id`.
...
- `incoming` filters by `addressee_user_id = actor_user_id`; `outgoing` filters by `requester_user_id = actor_user_id`.
