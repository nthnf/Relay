...
Friendship owns relationship-write state plus local replicated `user_account` read model. Other services must use friendship gRPC or friendship events instead of reading these tables directly.

## Replicated User Model

### `user_account`

Friendship-local copy of identity account existence and email verification state.
...
- Friendship must verify `addressee_user_id` existence through local `user_account` mirror before inserting the row.
...
- Friendship must verify `blocked_user_id` existence through local `user_account` mirror before inserting the row.
