...
Friendship owns relationship-write state plus local replicated `user_snapshot` read model. Other services must use friendship gRPC or friendship events instead of reading these tables directly.

## Replicated User Model

### `user_snapshot`

Friendship-local copy of identity account existence, verification state, and profile snapshot.
...
- Friendship must verify `addressee_user_id` existence through local `user_snapshot` mirror before inserting the row.
...
- Friendship must verify `blocked_user_id` existence through local `user_snapshot` mirror before inserting the row.
...
- `friend_request.requester_user_id -> user_snapshot.user_id`
- `friend_request.addressee_user_id -> user_snapshot.user_id`
- `friendship_edge.user_id -> user_snapshot.user_id`
- `friendship_edge.friend_user_id -> user_snapshot.user_id`
- `user_block.blocker_user_id -> user_snapshot.user_id`
- `user_block.blocked_user_id -> user_snapshot.user_id`

## Cross-Service References

- `identity` owns stable `user_id` namespace and publishes account lifecycle events consumed by friendship.
- Friendship validates write-path target users through local `user_snapshot` mirror and rejects unknown users instead of persisting orphaned relationship rows.
