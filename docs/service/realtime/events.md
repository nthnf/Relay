...
When chat or workspace call realtime directly, they should use the same logical event envelope as durable broker delivery: `delivery_id`, `target_kind`, `target_id`, `payload oneof`, `occurred_at`.
...
- Recovery path when synchronous `DeliverMessage` did not reach all intended connected sessions.
...
- `delivery_id` is the logical dedupe key across direct gRPC and RabbitMQ paths.
...
- In v1, the direct hot path is `DeliverMessage` for message-create fanout.
- Edits and deletes converge only through durable event consumption in v1.
...
- Duplicate websocket deliveries are allowed in rare races; clients should dedupe by `delivery_id`.
