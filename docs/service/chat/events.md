...
- After a reaction add commits successfully.
...
- `realtime` repair or catch-up fanout
- Analytics or notification workflows that care about reaction activity

### `MessageReactionRemoved`

**When published**

- After a reaction removal commits successfully.
...
- Reaction events describe membership changes for a single `(message_id, reaction_key, user_id)` tuple; downstream consumers must aggregate counts and user sets themselves.
