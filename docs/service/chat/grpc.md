...
Chat exposes hot-path message-write commands, bounded channel and direct-message history reads, reaction mutations, and minimal 1:1 DM conversation lifecycle reads/writes. `gateway` is the primary caller for end-user send, edit, delete, history, reaction, and DM-open flows.

## Shared Contract Rules

- Authenticated actor identity comes from `gateway`; callers must not mutate another actor's message state by supplying arbitrary user IDs.
...
- Validate DM targets by confirming the actor is an active `direct_conversation_member` of `direct_conversation_id`.
- If `client_message_id` is present, treat `(actor_user_id, channel_id, client_message_id)` or `(actor_user_id, direct_conversation_id, client_message_id)` as the idempotency key for the chosen target.
...
- Validate the actor can read the target channel through workspace-owned membership state.
...
- Reject self-DM creation in v1 unless later documented; `actor_user_id` and `peer_user_id` must differ.
- Return the existing durable conversation when one already exists for the unordered participant pair.
- Derive a canonical `pair_key` from the sorted `(actor_user_id, peer_user_id)` values and use it as the lookup/upsert key.
...
- Validate the actor is an active member of the direct conversation.
- Order results by `target_message_seq` descending for recent-history pagination unless a later doc revision defines otherwise.
...
- Require the actor to have access to the parent message target.
...
- Remove or deactivate only the actor's own matching reaction unless a later privileged moderation path is documented.
- The parent message may belong to either a workspace channel or a direct conversation; chat must authorize reaction changes against the owning target.
