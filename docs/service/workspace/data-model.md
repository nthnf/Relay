...
| `slug` | `text null` | Optional workspace-friendly identifier if later exposed at the edge. |
...
| `permissions` | `jsonb` | Contracted permission set for later authorization checks. |
...
- V1 may seed a system role for the creator during workspace creation even if role-management RPCs are deferred.
...
- Channel uniqueness within a workspace is explicit: `(workspace_id, name)` should be unique for active channels.
