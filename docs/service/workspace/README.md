...
- Mirror identity profile rows into workspace-local `user_snapshot` state for write-path target validation.
...
- **identity** as the owner of stable `user_id` references used by membership and invitation records, and as source for workspace-local `user_snapshot` mirror rows.
...
- Workspace keeps local `user_snapshot` rows in sync from identity events for write-path validation.
...
- Workspace validates direct-add and invitation targets against local `user_snapshot` rows, not by synchronously calling `identity`.
