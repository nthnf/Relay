...
Email owns outbound email intent handling, provider handoff, delivery attempts, and email-related operational state for registration verification. In v1 it is a consume-only service on the durable event bus.

## Owned Responsibilities

- Consume durable verification-email events from RabbitMQ.
- Materialize one service-owned `outbound_email` row per accepted email send intent.
- Render concrete verification messages from service-owned template/version metadata and self-contained consumed-event payloads.
...
- Owning user accounts or verification-token issuance; `identity` and `workspace` own those writes.
...
- **RabbitMQ** for durable consumption of registration verification events.
