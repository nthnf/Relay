...
use relay_proto::workspace::{CreateChannelRequest, CreateWorkspaceRequest, ListChannelsRequest};
...
async fn list_channels_returns_sorted_channels_for_member() -> Result<(), Box<dyn std::error::Error>>
{
    ...
    assert_eq!(listed.channels.len(), 2);
    assert_eq!(listed.channels[0].position, 1);
    assert_eq!(listed.channels[1].position, 2);
    assert_eq!(listed.channels[0].name, "general");
    assert_eq!(listed.channels[1].name, "general");
    assert_eq!(listed.channels[1].channel_id, extra.channel_id);
...
#[tokio::test]
async fn list_channels_rejects_non_member() -> Result<(), Box<dyn std::error::Error>> {
    let env = TestEnv::start().await?;
    let actor_user_id = Uuid::new_v4();
