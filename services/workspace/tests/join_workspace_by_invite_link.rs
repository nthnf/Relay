...
#[tokio::test]
async fn join_workspace_by_invite_link_creates_membership_and_updates_use_count()
-> Result<(), Box<dyn std::error::Error>> {
    let env = TestEnv::start().await?;
    let creator_user_id = Uuid::new_v4();
    let joiner_user_id = Uuid::new_v4();

    insert_user_snapshot(&env.db, creator_user_id).await?;
    insert_user_snapshot(&env.db, joiner_user_id).await?;
    ...
    let response = env
        .client
        .clone()
        .join_workspace_by_invite_link(actor_request(
            joiner_user_id,
            JoinWorkspaceByInviteLinkRequest {
    ...
    let member = workspace_member::Entity::find()
        ...
        .filter(workspace_member::Column::UserId.eq(joiner_user_id))
        .one(&env.db)
        .await?
        .expect("member row");
    assert_eq!(member.membership_status, "active");
    assert_eq!(member.added_by_user_id, Some(joiner_user_id));

    let invite_link = workspace_invite_link::Entity::find_by_id(link_id)
        .one(&env.db)
        .await?
        .expect("invite link row");
    assert_eq!(invite_link.use_count, 1);
    assert_eq!(invite_link.status, "active");

    let outbox_row = outbox_event::Entity::find()
        .filter(outbox_event::Column::AggregateType.eq("workspace_member"))
        ...
        .one(&env.db)
        .await?
        .expect("member added outbox row");
    assert_eq!(outbox_row.payload["source"], "invite_link");
    assert_eq!(outbox_row.payload["user_id"], joiner_user_id.to_string());
...
#[tokio::test]
async fn join_workspace_by_invite_link_rejects_expired_link()
-> Result<(), Box<dyn std::error::Error>> {
    let env = TestEnv::start().await?;
    let creator_user_id = Uuid::new_v4();
    let joiner_user_id = Uuid::new_v4();
    ...
    env.db
        .execute_unprepared(&format!(
            "UPDATE workspace_invite_link SET expires_at = '{}' WHERE id = '{}'",
            (chrono::Utc::now() - Duration::hours(1)).to_rfc3339(),
            link_id,
        ))
        .await?;
    ...
    let expired = env
        ...
        .join_workspace_by_invite_link(actor_request(
            joiner_user_id,
            JoinWorkspaceByInviteLinkRequest { code: link.code },
        ))
        .await;
