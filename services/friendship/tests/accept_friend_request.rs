...
use friendship::entity::{friend_request, outbox_event, user_block};
use sea_orm::{ColumnTrait, EntityTrait, QueryFilter, Set};

use setup::{accept_request, connect_friendship, FakeIdentityEnv, TestDbEnv};

#[tokio::test]
async fn rejects_accept_when_blocked() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    user_block::Entity::insert(user_block::ActiveModel {
        blocker_user_id: Set(requester_user_id),
        blocked_user_id: Set(addressee_user_id),
        created_at: Set(chrono::Utc::now().into()),
        reason: Set(None),
    })
    .exec(&db_env.db)
    ...
    let error = client
        .accept_friend_request(accept_request(addressee_user_id, friend_request_id))
        .await
        .expect_err("blocked accept should fail");
