...
use setup::{block_request, connect_friendship, unblock_request, FakeIdentityEnv, TestDbEnv};

#[tokio::test]
async fn unblocks_user_removes_block_and_emits_event()
-> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let response = client
        .unblock_user(unblock_request(actor_user_id, target_user_id))
        .await?
        .into_inner();

    assert!(response.unblocked);
    assert!(response.unblocked_at.is_some());

    assert!(
        user_block::Entity::find()
    ...
    assert!(
        outbox_event::Entity::find()
            .filter(outbox_event::Column::EventType.eq("UserUnblocked"))
            .one(&db_env.db)
            .await?
            .is_some()
    );
...
#[tokio::test]
async fn unblocks_missing_block_is_noop_without_event()
-> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    let actor_user_id = uuid::Uuid::new_v4();
    let target_user_id = uuid::Uuid::new_v4();
    let (identity_env, identity) = FakeIdentityEnv::start([target_user_id]).await?;
    ...
    assert!(!response.unblocked);
    assert!(response.unblocked_at.is_none());
    assert!(
        outbox_event::Entity::find()
            .filter(outbox_event::Column::EventType.eq("UserUnblocked"))
            .one(&db_env.db)
