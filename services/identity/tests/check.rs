...
use common::{TestEnv, access_token, insert_user_account, insert_user_profile, insert_user_session};
        ...
        attributes: Some(AttributeContext {
            request: Some(AuthRequest {
                http: Some(HttpRequest {
                    headers: std::iter::once(("authorization".to_string(), format!("Bearer {token}")))
                        .collect(),
                    ..Default::default()
                }),
                ..Default::default()
...
async fn check_accepts_active_session() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let headers = ok_response
        .headers
        .iter()
        .filter_map(|header| header.header.as_ref().map(|value| (value.key.as_str(), value.value.as_str())))
        .collect::<HashMap<_, _>>();

    assert_eq!(status.code, tonic::Code::Ok as i32);
    assert_eq!(status.message, "request is valid");
    assert_eq!(headers.get("x-user-id").copied(), Some(user_id.to_string().as_str()));
    assert_eq!(headers.get("x-session-id").copied(), Some(session_id.to_string().as_str()));
...
async fn check_rejects_unknown_user() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let response = env.auth_client.clone().check(check_request(&token)).await?.into_inner();
...
async fn check_rejects_inactive_account() -> Result<(), Box<dyn std::error::Error>> {
    ...
    insert_user_account(&env.db, user_id, "alice@example.com", Some(now), "disabled", now).await?;
    insert_user_session(
        &env.db,
        session_id,
        user_id,
